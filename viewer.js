...
function radarLoaded(r) {
  ...
  // Use server's legend (generated by mayara-core based on radar's pixel_values)
  const legend = convertServerLegend(r.legend);
...
// Convert server-provided legend to renderer format
// Server sends: [{type: "level_0", color: "#000000ff"}, ...]
// Renderer expects: [[r, g, b, a], ...]
function convertServerLegend(serverLegend) {
  const legend = [];

  // Convert each server legend entry to RGBA array
  for (const entry of serverLegend) {
    legend.push(hexToRGBA(entry.color));
  }

  // Pad to 256 entries for the renderer (values beyond pixelValues show as red)
  while (legend.length < 256) {
    legend.push([255, 0, 0, 255]); // Saturated red for overflow
  }
